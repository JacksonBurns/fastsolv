...
There are 99 unique solvents in the dataset, which is quite data-rich relative to other solubility datasets, which sometimes contain only single digit solvents, as in Boobier et al. (4 solvents), for example.[@boobier202machine] There are 165 unique solutes in the dataset. *Not sure how to refer to figures* Figure 1 below shows the distribution of logS across the dataset, ranging from -8.8 to 2.0. The distribution is left-skewed with a unimodal peak near -1. 

![label_distribution.\label{label_distribution}](../label_distribution.png){ width=2in }

Although 6236 datapoints is a relatively small dataset in the context of machine learning, this dataset is one of the largest available experimental solubility datasets in literature, so it is suitable to use in this molecular property prediction task.
# Methods
